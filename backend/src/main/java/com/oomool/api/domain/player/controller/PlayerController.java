...
public class PlayerController {
    ...
    @GetMapping("/{roomUID}")
    public ApiResponse<?> getPlayerList(@PathVariable("roomUID") String roomUid) {
        return ApiResponse.success(playerService.getPlayerDtoList(roomUid));
    }

    @Operation(summary = "플레이어의 마니띠 프로필 조회", description = "'나'와 나의 마니띠 프로필을 조회합니다.")
    @GetMapping("/{roomUID}/{userId}/manitti")
    public ApiResponse<?> getPlayerManitti(@PathVariable("roomUID") String roomUid,
        @PathVariable("userId") int userId) {
        return ApiResponse.success(playerService.getManittiPlayerProfile(roomUid, userId));
    }

    @Operation(summary = "플레이어의 마니또 프로필 조회", description = "'나'는 나의 마니또 결과를 조회합니다.")
    @GetMapping("/{roomUID}/{userId}/manitto")
    public ApiResponse<?> getMyManittoProfile(@PathVariable("roomUID") String roomUid,
        @PathVariable("userId") int userId) {
        return ApiResponse.success(playerService.getManittoPlayerProfile(roomUid, userId));
    }

    @Operation(summary = "플레이어가 예측한 결과(성공, 실패) 저장", description = "나는 마니또를 예측하고 그 결과를 저장합니다.")
    @PostMapping("/{roomUID}/{userId}/guess")
    public ApiResponse<?> saveGuessResult(@PathVariable("roomUID") String roomUid,
        ...
        return ApiResponse.success("추측 결과를 저장했습니다.");
    }
}
