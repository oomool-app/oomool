<template>
  <div class="pl-6 pr-6">
    ...
    <div class="input-container flex justify-center">
      <AlertDialog>
        <AlertDialogTrigger>
          <Button class="mt-6 mb-6" @click="check()">수정하기</Button>
        </AlertDialogTrigger>
...
const writeFeedAnswer = async (): Promise<void> => {
  ...
  formData.append('feed_id', feedId.value as string);
  try {
    await $api.feeds.modifyFeedAnswer(data);
...
// 내가 쓴 피드 내용 찾기
const getMyFeedAnswer = async (): Promise<void> => {
  const data = { roomUid, sequence: sequence.value };
  const response = await $api.feeds.getAllFeedsByRoomUidAndSequence(data);
