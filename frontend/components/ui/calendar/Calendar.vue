...
// @ts-expect-error in this current version of v-calendar has the calendarRef instance, which is required to handle arrow nav.
const calendarRef = computed<InstanceType<typeof Calendar> | undefined>(
  () => datePicker.value?.calendarRef,
);

async function handleNav(direction: 'prev' | 'next'): Promise<void> {
  if (calendarRef.value === null || calendarRef.value === undefined) return;
...
.calendar
  .is-not-in-month:not(
    :has(.vc-highlight-content-solid),
